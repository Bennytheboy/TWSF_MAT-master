clear all; close all;
warning off; clc;
Excel_File = '09. Frequency Ramp Test.xlsx';
List = dir('*.out');
delete('*.emf');
[row,col] = size(List);
tRun = 35;
...
for i = 1:row
    ...
    PSSE_SYS_FREQ_DEVIATION = Results_Psse.Out(:,2)*50+50;
    ...
    subplot(4,2,2)
    plot(PSSE_TIME,PSSE_SYS_FREQ_DEVIATION, 'b', 'LineWidth', 1.5); axis auto;
    set(gca,'xlim',[0 tRun]); grid on;
    ylabel('FREQ (Hz)');
    title('SYS_FREQ','Interpreter','none');
